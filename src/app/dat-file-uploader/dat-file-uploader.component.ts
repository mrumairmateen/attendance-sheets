...
export class DatFileUploaderComponent {
	...
	processData(data: string) {
		...
		lines.forEach(line => {
			...
			if (parts.length > 2) {
				...
				const dayMonthYear = `${ attendanceDate.getDate() }-${ attendanceDate.getMonth() + 1 }-${ attendanceDate.getFullYear() }`; // Store 'day-month-year'
				
				if (!attendanceMap.has(empId)) {
					attendanceMap.set(empId, new Set());
				}
				
				attendanceMap.get(empId)?.add(dayMonthYear);
			}
		});
	...
	generateTableData(attendanceMap: Map<string, Set<string>>) {
		this.tableData = [];
		this.employees.forEach((employee: Employee) => {
			...
			const daysPresent = attendanceMap.get(empId.toString()) || new Set<string>();
			...
			this.dateRange.forEach((date: Date, index: number) => {
				...
				} else if (daysPresent.has(dayMonthYear)) {
